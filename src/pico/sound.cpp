...
unsigned char mz80_read(u32 a,  struct MemoryReadByte *w)  { return z80_read(a); }
void mz80_write(u32 a, u8 d, struct MemoryWriteByte *w) { z80_write(d, a); }
...
void z80_init(void)
{
  ...
  Cz80_Init(&CZ80);
  Cz80_Set_Fetch(&CZ80, 0x0000, 0x1fff, (u32)Pico.zram); // main RAM
  Cz80_Set_Fetch(&CZ80, 0x2000, 0x3fff, (u32)Pico.zram); // mirror
  Cz80_Set_ReadB(&CZ80, (u8 (*)(u32 address))z80_read); // unused (hacked in)
...
void z80_pack(unsigned char *data)
{
  ...
  *(int *)(data+4) = Cz80_Get_Reg(&CZ80, CZ80_PC);
  memcpy(data+8, &CZ80, (s32)&CZ80.BasePC - (s32)&CZ80);
#endif
}
...
void z80_unpack(unsigned char *data)
{
  ...
  if (*(int *)data == 0x00007a43) { // "Cz" save?
    memcpy(&CZ80, data+8, (s32)&CZ80.BasePC - (s32)&CZ80);
