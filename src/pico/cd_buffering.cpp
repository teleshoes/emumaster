...
void PicoCDBufferInit(void)
{
	...
	{
		tmp = (unsigned char *)realloc(cd_buffer, PicoCDBuffers * 2048 + 304);
		if (tmp != NULL) break;
		PicoCDBuffers >>= 1;
	}
...
{
/*	TODO 	int is_bin, offs, read_len, moved = 0;
	reads++;
	...
	if (PicoCDBuffers <= 0)
	{
		...
		pm_seek((pm_file *)Pico_mcd->TOC.Tracks[0].F, where_seek, SEEK_SET);
		pm_read(dest, 2048, (pm_file *)Pico_mcd->TOC.Tracks[0].F);
		return;
	}
	...
	if (offs >= 0 && offs < PicoCDBuffers)
	{
		...
		memcpy32((int*)dest, (int *)(cd_buffer + offs*2048), 2048/4);
		return;
	}
	...
	if (is_bin)
	{
		...
		for (; i < read_len - 1; i++)
		{
			pm_read(cd_buffer + i*2048, 2048 + 304, (pm_file *)Pico_mcd->TOC.Tracks[0].F);
		...
		pm_read(cd_buffer + i*2048, 2048, (pm_file *)Pico_mcd->TOC.Tracks[0].F);
		pm_seek((pm_file *)Pico_mcd->TOC.Tracks[0].F, 304, SEEK_CUR);
	}
	else
	{
		pm_read(cd_buffer, read_len*2048, (pm_file *)Pico_mcd->TOC.Tracks[0].F);
	}
	memcpy32((int *)dest, (int *) cd_buffer, 2048/4);
	prev_lba = lba;
